...
def update_real_time_risk_chart(_):
    ...
    df = pd.read_sql_query("SELECT vehicle_id, risk_score FROM risk_scores", conn)
    conn.close()
...
def update_high_risk_alerts_chart(_):
    ...
    df = pd.read_sql_query("SELECT vehicle_id, risk_score FROM risk_scores WHERE risk_score = 'High Risk - Immediate action required'", conn)
    conn.close()
